...
class ContentPipeline:
    ...
    async def _generate_titles(
        ...
        *,
        ...
        system = (
            "你是中文标题策划。输出 JSON："
            '{"titles":[{"text":"","style":"搜索问题型|人群痛点型|误区纠正型|对比选择型|结果承诺型|数字结果悬念型|情绪共鸣型|故事型"}]}\n'
            f"栏目标题风格要求: {title_style_hint}\n"
            "高打开率三要素（每条尽量覆盖 2/3 以上）：\n"
            "1) 社交币：纠误区/反常识，读者愿转发收藏（如「别再…」「很多人不知道…」）\n"
            "2) 悬念：问句、数字未揭晓、转折「却/原来」（如「…？先查这3件事」）\n"
            "3) 利益点：人群+痛点+可执行结果，读者知道看了能得到什么\n"
            "优先模板：数字+结果+悬念。至少 30% 标题使用该模板。\n"
            "优化路径示例：\n"
            "- 普通「如何提高学习能力」→「普通学生怎么逆袭？学姐说的先做起来，其实是最有效的方法」\n"
            "- 普通「AI来了程序员失业」→「AI会让程序员失业吗？选专业前这3个判断」\n"
            "硬性要求：≤64字；含具体场景/人群/动作词；"
            "避免空泛词「提高」「全面」「指南」「必看」；不用震惊体、哗众取宠词。"
        )
        if style_weights:
            system += f"\n{style_weights}"
        user = (
